...
static const char* connectionString = "HostName=GoToCentre.azure-devices.net;DeviceId=movement_control;SharedAccessKey=NaFFeFIRA9KlAm4qZs7p2zODXk1bc9qbBf6TWpJsaCw=";
 
static const char* deviceId         = "movement_control"; /*must match the one on connectionString*/
...
Thread motor_thread(osPriorityNormal, 8*1024, NULL, "motor_thread");
...
DigitalOut   motor1_rev(PG_0);
DigitalOut   motor1_for(PG_1);
DigitalOut   motor2_rev(PE_0);
DigitalOut   motor2_for(PB_11);
...
//-------------------------------------------------------------------message to send to the board--------------------------------------------------------------
    ...
    if(!strcmp(temp,"go-forward"))
    {
        motor1_for=1;
        motor2_for=1;
    }
    if(!strcmp(temp,"go-backwards"))
        motor1_rev=motor2_rev=1;
    if(!strcmp(temp,"stop"))
        motor1_for=motor2_for=motor1_rev=motor2_rev=0;
    if(!strcmp(temp,"left-turn"))
    {
        motor1_for=1;
        motor2_for=motor2_rev=0;
    }
    if(!strcmp(temp,"right-turn"))
    {
        motor2_for=1;
        motor1_for=motor1_rev=0;
    }
